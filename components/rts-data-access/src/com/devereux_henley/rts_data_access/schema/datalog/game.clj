...
(def schema
  ...
  {;; ─── Game ────────────────────────────────────────────────────────────────
   ...
   ;; One :unit-statistics entity per patch the unit has been observed in.
   ;; Views pick the active one by joining through :unit-statistics/patch.
   ...
   ;; ─── Patch ──────────────────────────────────────────────────────────────
   ;; Engine patch (e.g. "4.0.1") that a given :unit-statistics snapshot
   ;; was scraped against. The seed loader inserts one :patch per RPFM
   ;; run and links every freshly scraped :unit-statistics to it.
   ;;
   ;; `:patch/version` is the display string. Use `:patch/released-at`
   ;; (not the version string) to order patches — lexicographic
   ;; comparison on semver-style strings is wrong as soon as any
   ;; component reaches double digits (`"4.10.0"` sorts before
   ;; `"4.2.0"`). The canonical "latest stats for unit X" query is:
   ;;
   ...
   ;;              db unit-eid)
   ;;
   ;; For the common "everyone's on the current patch" view, take the
   ;; global max once and pass it in as a filter:
   ;;
   ...
   ;; ─── Unit statistics ────────────────────────────────────────────────────
   ;; Per-patch snapshot of a unit's combat statistics. Replaces the
   ;; opaque JSON blob the SQLite era stored in `unit.unit_statistics`.
   ...
   ;; Engine keys (strings) — rendered directly by the unit template.
   ...
   :unit-statistics/attributes             {:db/valueType   :db.type/string
                                            :db/cardinality :db.cardinality/many}
   ;; Dynamic numeric stats ("armor", "leadership", "speed", …) modelled
   ;; as sub-entities so new patches can introduce stats without a schema
   ;; migration.
   :unit-statistics/stats                  {:db/valueType   :db.type/ref
                                            :db/cardinality :db.cardinality/many}
   ;; Equipment retains the engine's "vector of arbitrary maps" shape via
   ;; an EDN-serialized string until a domain epic needs to query into it;
   ;; flattening it now would be speculative.
   :unit-statistics/equipment              {:db/valueType :db.type/string}

   ;; ─── Unit stat sub-entity ──────────────────────────────────────────────
   ;; One per (unit-statistics, stat name) pair. No public eid — nothing
   ;; routes to a single stat — so the entity is transacted inline via
   ;; nested-map form under `:unit-statistics/stats`.
   :unit-stat/key                          {:db/valueType :db.type/string}
   ;; Value stored as a string because the engine emits some stats as
   ;; numbers ("armor": 80) and others as strings ("ammunition": "30").
