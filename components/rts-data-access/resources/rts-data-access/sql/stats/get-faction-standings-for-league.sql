...
FROM (
  ...
  WHERE l.eid = ? AND mg.winner_sub IS NOT NULL AND mg.player_one_draft_id IS NOT NULL
  UNION ALL
  ...
  FROM match_game mg
    ...
    INNER JOIN tournament t ON t.id = m.tournament_id
    INNER JOIN league l     ON l.id = t.league_id
  WHERE l.eid = ? AND mg.winner_sub IS NOT NULL AND mg.player_two_draft_id IS NOT NULL
) AS player_drafts
