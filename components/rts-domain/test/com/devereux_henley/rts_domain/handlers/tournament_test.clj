...
(deftest get-tournaments-assigns-type-to-each
  (with-redefs [data-access.contract/get-tournaments (fn [_ _ _ _] [{:eid test-tournament-eid :title "Tournament A"}
...
(deftest get-tournaments-empty-result
  (with-redefs [data-access.contract/get-tournaments (fn [_ _ _ _] [])]
...
(deftest get-tournaments-by-game-eid-assigns-type-to-each
  (with-redefs [data-access.contract/get-tournaments-by-game-eid (fn [_ _ _ _ _] [{:eid test-tournament-eid :title "Tournament A"}])]
...
(deftest get-tournaments-by-game-eid-empty-result
  (with-redefs [data-access.contract/get-tournaments-by-game-eid (fn [_ _ _ _ _] [])]
...
(deftest get-tournament-by-eid-assigns-type
  (with-redefs [data-access.contract/get-tournament-by-eid (fn [_ _] {:eid test-tournament-eid :title "Tournament A"})]
...
(deftest get-tournament-snapshot-by-eid-assigns-type
  (let [snapshot-eid (UUID/randomUUID)]
    (with-redefs [data-access.contract/get-tournament-snapshot-by-eid (fn [_ _] {:eid snapshot-eid :tournament-state "active"})]
...
(deftest get-tournament-snapshot-by-eid-nil-when-not-found
  (with-redefs [data-access.contract/get-tournament-snapshot-by-eid (fn [_ _] nil)]
...
(deftest get-tournament-snapshot-by-tournament-eid-assigns-type
  (with-redefs [data-access.contract/get-tournament-snapshot-by-tournament-eid (fn [_ _] {:eid (UUID/randomUUID) :tournament-state "active"})]
...
(deftest get-tournament-snapshot-by-tournament-eid-nil-when-not-found
  (with-redefs [data-access.contract/get-tournament-snapshot-by-tournament-eid (fn [_ _] nil)]
...
(deftest create-tournament-assigns-type
  (with-redefs [data-access.contract/create-tournament (fn [_ spec] spec)]
...
(deftest create-tournament-injects-created-at-timestamp
  (let [captured (atom nil)]
    (with-redefs [data-access.contract/create-tournament (fn [_ spec] (reset! captured spec) spec)]
...
(deftest create-tournament-preserves-specification-fields
  (let [captured (atom nil)
    ...
    (with-redefs [data-access.contract/create-tournament (fn [_ s] (reset! captured s) s)]
...
(deftest update-tournament-snapshot-assigns-type
  (with-redefs [data-access.contract/update-tournament-snapshot-by-tournament-eid (fn [_ _ snapshot] snapshot)]
    (let [result (handlers.tournament/update-tournament-snapshot
                  test-deps
                  test-tournament-eid
